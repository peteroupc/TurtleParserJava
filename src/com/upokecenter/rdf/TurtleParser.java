...
public class TurtleParser implements IRDFParser {
	...
	private static final RDFTerm RDF_TYPE=RDFTerm.fromIRI(
			"http://www.w3.org/1999/02/22-rdf-syntax-ns#type");
	private static final RDFTerm RDF_FALSE=RDFTerm.fromTypedString(
			"false","http://www.w3.org/2001/XMLSchema#boolean");
	private static final RDFTerm RDF_TRUE=RDFTerm.fromTypedString(
			"true","http://www.w3.org/2001/XMLSchema#boolean");
	private static final RDFTerm RDF_FIRST=RDFTerm.fromIRI(
			"http://www.w3.org/1999/02/22-rdf-syntax-ns#first");
	private static final RDFTerm RDF_REST=RDFTerm.fromIRI(
			"http://www.w3.org/1999/02/22-rdf-syntax-ns#rest");
	private static final RDFTerm RDF_NIL=RDFTerm.fromIRI(
	...
	private void emitRDFTriple(RDFTerm subj, RDFTerm pred, 
		...
		if(obj.kind==TurtleObject.SIMPLE){
			...
			if(objs.size()==0){
				...
				emitRDFTriple(curBlank,RDF_FIRST,objs.get(0),triples);
				for(int i=1;i<=objs.size();i++){
					if(i==objs.size()){
						...
						emitRDFTriple(nextBlank,RDF_FIRST,objs.get(i),triples);
						curBlank=nextBlank;
					}
				}
